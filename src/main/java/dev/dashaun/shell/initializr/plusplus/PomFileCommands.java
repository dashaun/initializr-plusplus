...
public class PomFileCommands {
  ...
  @ShellMethod("Add Spring Java Format Plugin")
  @ShellMethodAvailability("pomFile")
  public String springJavaFormatPlugin(){
    try {
      MavenXpp3Reader reader = new MavenXpp3Reader();
      Model model = reader.read(new FileReader(POM_FILE));
      Plugins.addJavaFormatPlugin(model);
      MavenXpp3Writer writer = new MavenXpp3Writer();
      writer.write(new FileWriter(POM_FILE), model);
    }catch(XmlPullParserException | IOException e){
      return "There was a problem adding spring-java-format-plugin.";
    }
    return "Successfully added spring-java-format-plugin.";
  }

  @ShellMethod("Add OpenRewrite recipes.")
  @ShellMethodAvailability("pomFile")
  public String rewritePlugin() {
    try {
      MavenXpp3Reader reader = new MavenXpp3Reader();
      Model model = reader.read(new FileReader(POM_FILE));

      //Update plugins
      Plugins.addOpenRewritePlugin(model);
    ...
    return "Successfully added openrewrite-plugin.";
  }
