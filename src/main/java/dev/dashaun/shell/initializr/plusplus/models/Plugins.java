...
public class Plugins {
  ...
  private final static String OPENREWRITE_GROUP_ID = "org.openrewrite.maven";
  private final static String OPENREWRITE_MAVEN_PLUGIN = "rewrite-maven-plugin";
  private final static String OPENREWRITE_PLUGIN_VERSION = "4.40.0";
  ...
  public static void addOpenRewritePlugin(Model model) {
    model.getBuild().getPlugins().removeIf(p -> p.getGroupId().equalsIgnoreCase(OPENREWRITE_GROUP_ID) && p.getArtifactId().equalsIgnoreCase(OPENREWRITE_MAVEN_PLUGIN));
    model.getBuild().getPlugins().add(Plugins.openRewritePlugin());
  }
  
  public static void addJavaFormatPlugin(Model model){
    model.getBuild().getPlugins().removeIf(p -> p.getGroupId().equalsIgnoreCase("io.spring.javaformat") && p.getArtifactId().equalsIgnoreCase("spring-javaformat-maven-plugin"));
    model.getBuild().getPlugins().add(Plugins.javaFormatPlugin());
  }
  ...
  public static Plugin openRewritePlugin() {
    Plugin p = new Plugin();
    p.setGroupId(OPENREWRITE_GROUP_ID);
    p.setArtifactId(OPENREWRITE_MAVEN_PLUGIN);
    p.setVersion(OPENREWRITE_PLUGIN_VERSION);
    Dependency rewriteSpring = new Dependency();
    rewriteSpring.setGroupId("org.openrewrite.recipe");
    rewriteSpring.setArtifactId("rewrite-spring");
    rewriteSpring.setVersion("4.32.0");
    p.setDependencies(Collections.singletonList(rewriteSpring));
    Xpp3Dom configuration = new Xpp3Dom("configuration");
    Xpp3Dom activeRecipes = new Xpp3Dom("activeRecipes");
    String[] recipes = {"org.openrewrite.java.format.AutoFormat",
    "org.openrewrite.java.spring.boot2.SpringBoot2JUnit4to5Migration"};
    for (String i : recipes) {
      Xpp3Dom recipe = new Xpp3Dom("recipe");
      recipe.setValue(i);
      activeRecipes.addChild(recipe);
    }
  ...
  private static PluginExecution validateFormat() {
    PluginExecution validateFormat = new PluginExecution();
    validateFormat.setId("validate");
    validateFormat.setPhase("validate");
    validateFormat.setInherited(true);
    validateFormat.getGoals().add("validate");
